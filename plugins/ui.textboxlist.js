(function($) {
  ...
  $.widget("ui.textboxlist", $.extend({}, $.ui.coreext, {
    ...
    _create_bubble : function( $li, value ) {
      ...
      var $remove     = $('<span class="ui-textboxlist-deletebutton" />'),
          class_count = 0,
          codes       = this.keycodes,
          ptn         = '';
      ...
      $li.html( '<span class="ui-textboxlist-selection-display">' + value + '</span>' );

      $li.append( $remove );

      for ( class_count; class_count < this.options.adtl_close_classes.length; ++class_count ) {
        $remove.addClass( this.options.adtl_close_classes[ class_count ] );
      }

      $li.addClass( 'ui-textboxlist-bit-done' );

      for ( class_count = 0; class_count < this.options.adtl_done_classes.length; ++class_count ) {
        $li.addClass( this.options.adtl_done_classes[ class_count ] );
      }

      $remove.bind( 'click', { textboxlist: this }, this._remove_bubble );
    ...
    _remove_bubble : function( e ) {
      ...
      var $parent     = $(this).closest('li'),
          textboxlist = e.data.textboxlist;
